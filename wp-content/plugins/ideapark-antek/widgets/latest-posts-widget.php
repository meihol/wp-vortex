...
class Ideapark_Latest_Posts_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_options = [
			...
			'description' => esc_html__( 'The recent posts with thumbnails', 'ideapark-antek' )
		];

		parent::__construct(
			'ideapark_latest_posts_widget', esc_html__( 'Antek Latest Posts', 'ideapark-antek' ), $widget_options
		);
	}
				...
				<?php foreach ( $posts AS $post ) : ?>
					<li class="c-lp-widget__item">
						...
						<div class="c-lp-widget__content">
							...
							<div class="c-lp-widget__date"><?php esc_html_e('Posted:', 'ideapark-antek'); ?> <?php the_time( get_option( 'date_format' ) ); ?></div>
						</div>
					</li>
				<?php endforeach; ?>
	...
	function form( $instance ) {
		$defaults = [
			'title'         => esc_html__( 'Latest Posts', 'ideapark-antek' ),
			'post_per_page' => 5,
			'categories'    => ''
		];
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'ideapark-antek' ); ?></label>
			<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" value="<?php echo esc_attr($instance['title']); ?>" />
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'categories' ) ); ?>"><?php esc_html_e( 'Filter by Category:', 'ideapark-antek' ); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'categories' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'categories' ) ); ?>" class="widefat categories" style="width:100%;">
				<option value="all" <?php if ( $instance['categories'] == 'all' ) { ?> selected<?php } ?>><?php esc_html_e( 'All categories', 'ideapark-antek' ); ?>
				</option>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'post_per_page' ) ); ?>"><?php esc_html_e( 'Number of posts to show:', 'ideapark-antek' ); ?></label>
