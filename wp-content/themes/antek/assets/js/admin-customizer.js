(function ($, root, undefined) {
	...
	(function (api) {
		...
		$(document).ready(function () {
			...
			$('.js-ideapark-checkbox-set-item').on('click', function () {
				var choices = '';
				var $control = $(this).closest('.ideapark-checkbox-set-list');
				$control.find('input[type="checkbox"]').each(
					function () {
						if ($(this).get(0).checked) {
							choices += (choices ? ',' : '') + $(this).val();
						}
					}
				);
				$('.js-ideapark-checkbox-set',$control).val(choices).trigger('change');
			});
			...
			var ideapark_admin_refresh_checklist = function (container) {
				...
				container.find('input[type="checkbox"]').each(
					function () {
						choices += (choices ? '|' : '') + $(this).data('name') + '=' + ($(this).get(0).checked ? $(this).val() : '0');
					}
				);
							...
							$(this).sortable(
								{
									...
									update     : function (event, ui) {
										...
										ui.item.parent().find('input[type="checkbox"]').each(
											function () {
												choices += (choices ? '|' : '')
													+ $(this).data('name') + '=' + ($(this).get(0).checked ? $(this).val() : '0');
											}
										);
		...
		api.bind('ready', function () {
			...
			api.panel( 'theme_settings', function( panel ) {
				panel.expanded.bind( function( isExpanded ) {
					if ( isExpanded && ideapark_ac_vars.catalogUrl) {
						api.previewer.previewUrl.set( ideapark_ac_vars.catalogUrl );
					}
				} );
			} );
			
			api.section( 'ideapark_section_woocommerce', function( section ) {
			...
			api.section( 'ideapark_section_item_page', function( section ) {
				section.expanded.bind( function( isExpanded ) {
					if ( isExpanded && ideapark_ac_vars.itemUrl ) {
						api.previewer.previewUrl.set( ideapark_ac_vars.itemUrl );
					} else {
						api.previewer.previewUrl.set( ideapark_ac_vars.catalogUrl );
					}
				} );
			...
			api.section( 'ideapark_section_post_page', function( section ) {
				section.expanded.bind( function( isExpanded ) {
			...
			$('.customize-control-font-icons')
				.select2({
					...
					allowClear       : true,
					templateSelection: f_icon,
					templateResult   : f_icon
				});
		});
