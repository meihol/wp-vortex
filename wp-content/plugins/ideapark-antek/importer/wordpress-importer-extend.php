...
class WP_Importer_Extend extends WP_Import {
	...
	public function importing() {
		...
		add_filter( 'wp_import_post_meta', [ $this, 'wp_import_post_meta' ], 10, 3 );
		...
		do {
			...
			foreach ( $this->_posts as $index => $post ) {
				if ( $post['post_type'] == 'attachment' ) {
					...
					if ( ! $this->message ) {
						$this->message = __( 'Importing media files', 'ideapark-antek' );
					}
			...
			if ( ! $is_found ) {
				foreach ( $this->_posts as $index => $post ) {
					if ( $post['post_type'] != 'nav_menu_item' ) {
						...
						if ( ! $this->message ) {
							$this->message = __( 'Importing posts', 'ideapark-antek' );
						}
					}
				}
			}
			...
			if ( ! $is_found ) {
				foreach ( $this->_posts as $index => $post ) {
					...
					if ( ! $this->message ) {
						$this->message = __( 'Importing nav menus', 'ideapark-antek' );
					}
				}
			}
			...
			try {
				...
				$this->message = __( 'Error importing the post #', 'ideapark-antek' ) . $post['post_id'] . ': ' . $e->getMessage();
				break;
			}
				}
	...
	public function import_start( $file ) {
		...
		if ( is_wp_error( $fn ) ) {
			...
			if ( is_file( $fn2 = IDEAPARK_UPLOAD_DIR . 'content.xml' ) ) {
				...
				return new WP_Error( 'ideapark_file_not_found', __( 'File content.xml not found', 'ideapark-antek' ) );
			}
		}
	...
	public function wp_import_post_meta( $postmeta, $post_id, $post ) {
		foreach ( $postmeta AS $i => $meta ) {
			if ( $meta['key'] == 'location_drop' ) {
				if ( ! empty( $meta['value'] ) ) {
					$a = [];
					$e = explode( ',', $meta['value'] );
					foreach ( $e as $term_id ) {
						if ( array_key_exists( $term_id, $this->processed_terms ) ) {
							$a[] = $this->processed_terms[ $term_id ];
						}
					}
					$postmeta[ $i ]['value'] = implode( ',', $a );
				}
			}
		}
