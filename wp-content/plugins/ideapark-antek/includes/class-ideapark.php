...
class Ideapark_Antek {
	...
	public function __construct( $file = '', $version = '1.0.0' ) {
		...
		add_action( 'customize_controls_enqueue_scripts', [ $this, 'admin_enqueue_scripts' ], 10, 1 );
		add_action( 'customize_controls_enqueue_scripts', [ $this, 'admin_enqueue_styles' ], 10, 1 );
	...
	public function register_post_type( $post_type = '', $plural = '', $single = '', $description = '', $options = [] ) {
		...
		$post_type = new Ideapark_Antek_Post_Type( $post_type, $plural, $single, $description, $options );
	...
	public function register_taxonomy( $taxonomy, $object_type, $plural = '', $single = '', $description = '', $options = [] ) {
		...
		$post_type = new Ideapark_Antek_Taxonomy( $taxonomy, $object_type, $plural, $single, $description, $options );
			...
			if ( $screen && $screen->base == 'edit' ) {
				...
				if ( isset( $wp_query->query['post_type'] ) && ! is_array( $wp_query->query['post_type'] ) && in_array( $wp_query->query['post_type'], $this->sorted_post_types ) ) {
					if ( empty( $wp_query->query['suppress_filters'] ) ) {
						if ( $wp_query->get( 'orderby' ) == 'date' && $wp_query->get( 'order' ) == 'DESC' && ! ideapark_mod( '_disable_menu_order' ) ) {
							$wp_query->set( 'orderby', 'menu_order' );
							$wp_query->set( 'order', 'ASC' );
						}
					}
	...
	public function admin_enqueue_scripts( $hook = '' ) {
		global $ideapark_antek_sort_notice;
		wp_enqueue_media();
		...
		if ( $is_sortable_tax || $is_sortable_post_type ) {
			...
			if ( ! empty( $ideapark_antek_sort_notice ) && ( $index = $is_sortable_tax ? $_GET['taxonomy'] : $_GET['post_type'] ) && array_key_exists( $index, $ideapark_antek_sort_notice ) ) {
				wp_localize_script( $this->_token . '-sort', 'ideapark_sort_vars', [
					'notice' => esc_html( $ideapark_antek_sort_notice[ $index ] ),
				] );
			}
		}
	...
	public function install( $network_wide = false ) {
		...
		$install = function () {
			global $wpdb;
			if ( $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->terms}'" ) ) {
