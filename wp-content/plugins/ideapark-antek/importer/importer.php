...
class Ideapark_Importer {
	...
	function admin_menu() {
		add_theme_page( __( 'Import Demo Content', 'ideapark-antek' ), __( 'Import Demo Content', 'ideapark-antek' ), 'manage_options', 'ideapark_themes_importer_page', [
			$this,
			'importer_page'
		] );
	}
	...
	public function scripts( $hook ) {
		...
		wp_localize_script( 'ideapark-importer', 'ideapark_wp_vars_importer', [
			...
			'please_wait'  => __( 'Please wait...', 'ideapark-antek' ),
			'are_you_sure' => __( 'Are you sure you want to import?', 'ideapark-antek' ),
			'importing'    => __( 'Importing ...', 'ideapark-antek' ),
			'progress'     => __( 'Progress', 'ideapark-antek' ),
			'output_error' => __( 'Output Error', 'ideapark-antek' ),
		] );
	}
	...
	function importer_page() {
		...
		if ( is_wp_error( $wp_filesystem->errors ) && $wp_filesystem->errors->get_error_code() == 'empty_hostname' ) {
			...
			$output .= '<h1>' . IDEAPARK_NAME . ' - ' . __( 'One-Click Import Demo Content', 'ideapark-antek' ) . '</h1>';
		...
		if ( ! empty( $themes ) ) {
			$output   .= '<p><span class="subheader">' . __( 'Select the demo site you want to import: ', 'ideapark-antek' ) . '</span></p>';
			...
			foreach ( $themes as $name => $theme ) {
				$preview_button = '';
				if ( ! empty( $theme['url'] ) ) {
					$preview_button = '<a class="ip-demo-preview" href="' . esc_url( $theme['url'] ) . '" target="_blank">' . __( 'Preview Demo', 'ideapark-antek' ) . '</a>';
				}
		...
		if ( ! empty( $themes ) || IDEAPARK_DEMO ) {
			$output .= '<p' . ( ( ! $is_fonts && ! $is_revslider ) ? ' class="ip-invisible"' : '' ) . '><span class="subheader">' . __( 'Select the data you want to import: ', 'ideapark-antek' ) . '</span>' .
			           '<br />
			<label><input type="radio" name="import_option" value="all" checked/>' . __( 'All data', 'ideapark-antek' ) . '</label><br />
			';
			if ( $is_fonts ) {
				$output .= '<label class="ip-fonts-radio"><input type="radio" name="import_option" value="fonts"/>' . __( 'Fonts', 'ideapark-antek' ) . '</label><br />';
			}
			if ( $is_revslider ) {
				$output .= '<label class="ip-rev-slider-radio"><input type="radio" name="import_option" value="revslider"/>' . __( 'Slider Revolution', 'ideapark-antek' ) . '</label><br />';
			}
			$output .= '</p>';
			if ( PHP_OS == "Darwin" && IDEAPARK_DEMO ) {
				$output .= '<label><input type="radio" name="import_option" value="options"/>' . __( 'Options', 'ideapark-antek' ) . '</label><br />';
			}

			$output .= '<p><label><input type="checkbox" value="1" name="import_attachments" checked /> ' . __( 'Import attachments', 'ideapark-antek' ) . '</label></p>';
			if ( ( $code = ideapark_get_purchase_code() ) && $code !== IDEAPARK_SKIP_REGISTER ) {
				$output .= '<p class="submit">' . ( PHP_OS == "Darwin" || ! IDEAPARK_DEMO ? '<button class="button button-primary" id="ip-import-submit">' . __( 'Import', 'ideapark-antek' ) . '</button> ' : '' ) . '</p>';
				$output .= '<div class="ip-loading-progress"><div class="ip-loading-bar"><div class="ip-loading-state"></div><div class="ip-loading-info">' . __( 'Progress', 'ideapark-antek' ) . ': 0%...</div></div><div class="ip-import-output">' . __( 'Prepare data...', 'ideapark-antek' ) . '</div></div>';
				$output .= '<div class="ip-import-notes">';
				$output .= __( 'Important notes:', 'ideapark-antek' ) . '<br />';
				$output .= __( 'Please note that import process will take time needed to download all attachments from demo web site.', 'ideapark-antek' ) . '<br />';
				$output .= __( 'If you plan to use shop, please install WooCommerce before you run import.', 'ideapark-antek' ) . '<br />';
				$output .= sprintf( wp_kses( __( 'We recommend you to <a href="%s" target="_blank">reset data</a> & clean wp-content/uploads folder before import to prevent duplicate content.', 'ideapark-antek' ), [ 'a' => [ 'href' => [] ] ] ), esc_url( 'https://wordpress.org/plugins/wp-reset/' ) ) . '<br />';

				if ( ! ini_get( 'allow_url_fopen' ) ) {
					$output .= '<div class="error">' . esc_html__( 'Downloading remote files is prohibited in PHP (allow_url_fopen in php.ini)', 'ideapark-antek' ) . '</div>';
				} elseif ( ! isset( $_GET['download_test'] ) ) {
					$output .= '<p>' . esc_html__( 'If you have problems importing images, then run a test to get information about the error', 'ideapark-antek' ) . ': <a href="' . admin_url( 'themes.php?page=ideapark_themes_importer_page&download_test' ) . '">' . esc_html__( 'download test', 'ideapark-antek' ) . '</a></p>';
				} else {
					...
					if ( is_wp_error( $result ) ) {
						$output .= '<div class="error">' . esc_html__( 'Download test failed', 'ideapark-antek' ) . ': ' . $result->get_error_message() . '</div>';
					} else {
						$output .= '<p class="warning">' . esc_html__( 'Download test completed successfully', 'ideapark-antek' ) . '</p>';
				...
				$output .= '<p class="ideapark-antek-warning">' . esc_html__( 'Available only to registered theme', 'ideapark-antek' ) . '</p>';
				$output .= '<p><a class="button button-primary" href="' . admin_url( 'themes.php?page=ideapark_about' ) . '">' . __( 'Register theme', 'ideapark-antek' ) . '</a></p>';
				$output .= '<div class="ip-import-output"></div>';
			}
			if ( defined( 'IDEAPARK_DEMO' ) && IDEAPARK_DEMO ) {
				$output .= '<p><a href="" style="color: #F0F0F1;" onclick="return false;" id="ip-export-submit">' . __( 'Export', 'ideapark-antek' ) . '</a></p>';
			}
		} else {
			$output .= '<div>' . __( 'No themes data found.', 'ideapark-antek' ) . '</div>';
			$output .= '<div>' . __( 'Try adding this code to the beginning of the file wp-config.php:', 'ideapark-antek' ) . '</div>';
	...
	function importer() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			$this->import_response( 'error', __( 'Error: Permission denied', 'ideapark-antek' ) );
		}
		...
		if ( isset( $_REQUEST['stage'] ) && $_REQUEST['stage'] == 'start' ) {
			if ( ! empty( $_REQUEST['import_option'] ) && ! empty( $_REQUEST['import_demo'] ) ) {
				...
				if ( ! ideapark_is_dir( $theme_title_fn = $this->importer_dir . '/' . $this->demo_content_folder . '/' ) ) {
					$this->import_response( 'error', __( 'Error: Can`t open file: ', 'ideapark-antek' ) . $theme_title_fn );
				}
				...
				switch ( $_REQUEST['import_option'] ) {
					...
					default:
						$this->import_response( 'error', __( 'Error: Select the data you want to import', 'ideapark-antek' ) );
				}
		...
		if ( empty( $this->importer['steps'][0] ) ) {
			...
			if ( empty( $this->importer['steps'][0] ) ) {
				$this->import_response( 'error', __( 'Error: Select the data you want to import', 'ideapark-antek' ) . ( $this->is_file_transient ? ' (file)' : ' (db)' ) );
			}
		}

		if ( empty( $this->importer['import_demo_content_folder'] ) ) {
			$this->import_response( 'error', __( 'Error: Selected data is empty', 'ideapark-antek' ) . ( $this->is_file_transient ? ' (file)' : ' (db)' ) );
		}
		...
		switch ( $step ) {
			...
			case 'prepare':
				...
				$message                           = __( 'Prepared data successfully', 'ideapark-antek' );
			...
			case 'terms':
				...
				array_shift( $this->importer['steps'] );
				$this->importer['base']->step_done += $among;
				$message                           = __( 'Imported terms successfully', 'ideapark-antek' );
			...
			case 'post':
				...
				if ( ! $this->importer['base']->importing() ) {
					array_shift( $this->importer['steps'] );
					$message                           = __( 'Imported post data successfully', 'ideapark-antek' );
					$this->importer['base']->step_done += $among;
				} else {
			...
			case 'options':
				...
				array_shift( $this->importer['steps'] );
				$this->importer['base']->step_done += $among;
				$message                           = __( 'Imported options successfully', 'ideapark-antek' );
			...
			case 'fonts':
				...
				array_shift( $this->importer['steps'] );
				$this->importer['base']->step_done += $among;
				$message                           = __( 'Imported fonts successfully', 'ideapark-antek' );
			...
			case 'widget':
				...
				array_shift( $this->importer['steps'] );
				$this->importer['base']->step_done += $among;
				$message                           = __( 'Imported widgets successfully', 'ideapark-antek' );
			...
			case 'completed':
				...
				if ( ! count( $this->importer['base']->error_msg ) ) {
					$message = '<b style="color:#444">' . __( 'Cheers! The demo data has been imported successfully! Please reload this page to finish!', 'ideapark-antek' ) . '</b>';
				} else {
					$permalinks_settings_url = get_admin_url( null, 'options-permalink.php' );
					$message                 = '<b style="color:#444">' . __( 'Data import completed!', 'ideapark-antek' ) . '</b>' . '<p><a class="button" href="' . $permalinks_settings_url . '">' . __( 'Re-save the site permalinks! ', 'ideapark-antek' ) . '</a></p>' . '<div>' . implode( '', $this->importer['base']->error_msg ) . '</div>';
				}
				$code = 'completed';
			...
			default:
				$this->import_response( 'error', __( 'Error: step not found: ', 'ideapark-antek' ) . $step );
				break;
		}
					...
					if ( $step == 'prepare' ) {
						...
						if ( ! $_result || ! ideapark_is_file( $this->session_fn_start ) ) {
							if ( get_filesystem_method() !== 'direct' ) {
								$this->import_response( 'error', __( 'Error: No access to data folder. Try to add line "define(\'FS_METHOD\',\'direct\');" to the file wp-config.php', 'ideapark-antek' ) );
							} else {
								$this->import_response( 'error', __( 'Error: Uploads folder must be writable - ', 'ideapark-antek' ) . IDEAPARK_UPLOAD_DIR );
							}
						}
					}
				} catch ( Exception $e ) {
...
//				ideapark_fpc( $theme_options_fn . '.txt', base64_decode( $theme_options_txt ) );
				...
				$this->import_response( 'error', __( 'Error: file not found: ', 'ideapark-antek' ) . $theme_options_fn );
			}
		}
	...
	function import_revslider() {
		...
		if ( $this->slides ) {
			if ( class_exists( 'RevSlider' ) ) {
				...
				if ( ! $has_errors ) {
					$message = esc_html__( 'Imported Slider Revolution successfully', 'ideapark-antek' );
				}

			} else {
				$this->importer['base']->error_msg[] = $message = __( 'The plugin Slider Revolution is not installed', 'ideapark-antek' );
			}
		} else {
			$message = __( 'There are no slides in this demo', 'ideapark-antek' );
		}
	...
	function exporter() {
		...
		if ( ! ideapark_is_dir( $this->export_path ) ) {
			if ( ! ideapark_mkdir( $this->export_path ) ) {
				$this->import_response( 'error', __( 'Error: Permission denied', 'ideapark-antek' ) . ': ' . $this->export_path );
			}
		}
		...
		$code    = 'completed';
		$message = '<b style="color:#444">' . __( 'The demo data has been exported successfully!', 'ideapark-antek' ) . '</b>';
...
//			echo "<b>REV SLIDER</b>: " . $val_orig . ' ' . $data->settings->wp->rev_slider . '<br>';
	...
	function prepere_post_meta( $mode ) {
		global $wpdb;
		$postmeta = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->postmeta} WHERE meta_key = %s", $mode ? '_elementor_data_antek' : '_elementor_data' ) );
		foreach ( $postmeta as $meta ) {
			...
			if ( $mode ) {
				...
				delete_post_meta( $meta->post_id, '_elementor_data_antek' );
			} else {
//				update_post_meta( $meta->post_id, '_elementor_data_antek', '' );
//				$table = _get_meta_table( 'post' );
//				$wpdb->update( $table, [ 'meta_value' => wp_json_encode( $data ) ], [ 'post_id'  => $meta->post_id,
//				                                                                      'meta_key' => '_elementor_data_antek'
//				] );
				...
				update_post_meta( $meta->post_id, '_elementor_data_antek', wp_slash( wp_json_encode( $data ) ) );
			}
		}
	...
	function export_content() {
		...
		$wpdb->query( "DELETE FROM $wpdb->postmeta WHERE meta_key IN ('_elementor_data_antek', '_ideapark_variation_images')" );
	...
	public function on_wp_import_post_meta(
		...
		foreach ( $post_meta as &$meta ) {
			if ( '_elementor_data_antek' === $meta['key'] ) {
	...
	public function on_wxr_importer_pre_process_post_meta(
		...
		if ( '_elementor_data_antek' === $post_meta['key'] ) {
